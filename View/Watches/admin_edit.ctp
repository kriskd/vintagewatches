<div class="watches form">
	...
	<?php if(!empty($presenter['Image'])): ?>
		<div class="watch-images">
			<?php $images = $presenter['Image']; ?>
			<?php foreach($images as $image): ?>
				<div class="image-thumb">
					<?php echo $this->Html->link($this->Html->thumbImage($presenter['Watch']['id'], $image['filenameThumb']),
                                                 '#delete-' . $presenter['Watch']['id'] . '-' . $image['id'],
                                                 array('escape' => false, 'data-toggle' => 'modal')
                                                 );
					?>
                    <div class="modal fade" id="delete-<?php echo $presenter['Watch']['id']; ?>-<?php echo $image['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <?php echo $this->Html->thumbImage($presenter['Watch']['id'], $image['filenameThumb'],
                                                                        array('escape' => false)
                                                 ); ?>
                                </div>
					...
					<?php else: ?>
						<?php echo $this->Html->link('Make Primary',
								     array('controller' => 'images',
									   'action' => 'primary', $presenter['Watch']['id'], $image['id'],
									   'admin' => 'true')
                                     ); ?>
					<?php endif; ?>
    ...
    <div class="jumbotron">
        <?php echo $this->Form->create('Image', array('type' => 'file', 'url' => array('action' => 'upload', $presenter['Watch']['id']))); ?>
        ...
        <?php echo $this->Form->input('watch_id', array('type' => 'hidden', 'value' => $presenter['Watch']['id'])); ?>
